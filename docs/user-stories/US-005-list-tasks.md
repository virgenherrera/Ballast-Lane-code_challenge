...
- [x] API contract for GET /api/tasks frozen per docs/architecture/api-contract.md Section 4.2 and Section 8: query params (status, page, perPage), response shape { items: [{id, title, status, dueDate}], paging: {page, perPage, total, prev, next} }
- [x] Ordering rule confirmed: createdAt DESC with id DESC as deterministic tie-breaker for millisecond-collision stability across page boundaries
...
- [x] TaskListItemDto field set confirmed as distinct from TaskDetailDto: id, title, status, dueDate (no description, no ownerId, no timestamps)
- [x] ITaskRepository.ListAsync signature agreed: Task<(IReadOnlyList<TaskItem> Items, int Total)> ListAsync(Guid ownerId, TaskStatus? status, int page, int perPage, CancellationToken ct)
- [x] prev/next relative-URL format agreed per api-contract.md examples: '/api/tasks?status=Pending&page=1&perPage=20' with stable canonical query-param ordering for testability
- [x] Error shape { status, error, message, details: [{field, issue}] } confirmed as global contract per api-contract.md Section 2.3; FluentValidation validators must populate details consistently
- [ ] dueDate confirmed as DATE-ONLY (yyyy-MM-dd, nullable); createdAt/updatedAt as UTC ISO 8601 timestamps with 'Z' suffix

## Acceptance Criteria

- [x] **AC-005.1: Returns all own tasks with no filters**
...
- [x] **AC-005.2: Empty task list returns empty items, not an error**
...
- [x] **AC-005.3: Ownership isolation across multiple owners**
...
- [x] **AC-005.4: Item field set exposed by TaskListItemDto**
...
- [x] **AC-005.5: Default pagination when no params supplied**
...
- [x] **AC-005.6: Second page returns prev link and correct slice**
...
- [x] **AC-005.7: Page beyond last available page**
...
- [x] **AC-005.12: Deterministic ordering under timestamp collision**
...
- [x] All 10 listed test cases pass plus any new boundary tests added during implementation
- [x] GET /api/tasks implemented per Clean Architecture layering: Domain -> Application -> Infrastructure -> API
- [x] ListTasksQueryHandler implemented against ITaskRepository with unit tests using NSubstitute mock
- [x] TaskRepository.ListAsync implemented with EF Core LINQ only (no raw SQL), integration-tested against real PostgreSQL via Testcontainers
- [x] ListTasksQueryValidator uses FluentValidation with CascadeMode.Continue; rejects page < 1, perPage < 1, perPage > MaxPerPage with standard error shape containing field-level details
- [x] Response matches exact contract shape: { items, paging: { page, perPage, total, prev, next } }
- [x] Ordering by createdAt DESC, id DESC verified by explicit test with 3+ tasks including timestamp-collision scenario
- [x] prev/next URLs are relative, preserve status filter when present, and are null at first/last page boundaries
- [x] Ownership filter (WHERE ownerId = @currentOwnerId) present in query even though not yet JWT-driven
- [x] Domain layer has ZERO external package references; no EF/Npgsql types leak into Application or Domain
- [x] Swagger/OpenAPI updated for GET /api/tasks with query params and response schema
- [x] No InMemory/SQLite provider used anywhere in test project
